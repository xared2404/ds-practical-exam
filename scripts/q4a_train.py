...
def main():
    ...
    for i, cutoff in enumerate(cutoffs, start=1):
        ...
        print(f"[Q4A] Split {i}: cutoff={cutoff} train_n={len(train)} test_n={len(test)} test_balance={bal}")
    ...
    avg = metrics.groupby("model")[["accuracy","precision","recall","f1"]].mean().reset_index()
